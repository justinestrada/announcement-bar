...
class Announcement_Bar_Shortcodes
{
  ...
  public function announcement_bar_shortcode($atts, $content = null)
  {
    ...
    if (isset($this->settings->enable_auto_activate) && $this->settings->enable_auto_activate) {
      ...
      $auto_activate_date_time = (int) strtotime(date("Y-m-d H:i:s", strtotime($this->settings->auto_activate_date_time))) * 1000;
      $show_bar = ($current_date >= $auto_activate_date_time);
    }
    if (isset($this->settings->enable_auto_deactivate) && $this->settings->enable_auto_deactivate) {
      ...
      $current_date = (int) strtotime(date("Y-m-d H:i:s")) * 1000;
      $auto_deactivate_date_time = (int) strtotime(date("Y-m-d H:i:s", strtotime($this->settings->auto_deactivate_date_time))) * 1000;
